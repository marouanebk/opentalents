...
class CPTable(tables.Table):
    action = '{% load icons %}\
            ...
            {% endif %}'
    edit= tables.TemplateColumn(action, orderable=False)
    pv1 = tables.TemplateColumn('<a href="{% url "generate_pdf" pk=record.pk type="pv1" %}" class="btn btn-primary" download>Generate PV1</a>', orderable=False)
    pv2 = tables.TemplateColumn('<a href="{% url "generate_pdf" pk=record.pk type="pv2" %}" class="btn btn-primary" download>Generate PV2</a>', orderable=False)    
    class Meta:
        model = CP
        fields=('formation','periode', 'date_cp1','date_cp2', 'pv1', 'pv2')
        template_name= "django_tables2/bootstrap4.html"
...
class DelegueTable(tables.Table):
    formation = tables.Column()
    etudiants_count = tables.Column(empty_values=())

    class Meta:
        model = Delegue
        fields = ('formation', 'etudiants_count')
        template_name= "django_tables2/bootstrap4.html"

    def render_etudiants_count(self, record):
        return record.etudiants.count()
...
class CPTable(tables.Table):
    action = '{% load icons %}\
            ...
            {% endif %}'
    edit= tables.TemplateColumn(action, orderable=False)
    pv1 = tables.TemplateColumn('<a href="{% url "generate_pdf" pk=record.pk type="cp1" %}" class="btn btn-primary" download>Generate PV1</a>', orderable=False)
    pv2 = tables.TemplateColumn('<a href="{% url "generate_pdf" pk=record.pk type="cp2" %}" class="btn btn-primary" download>Generate PV2</a>', orderable=False)    
    class Meta:
        model = CP
